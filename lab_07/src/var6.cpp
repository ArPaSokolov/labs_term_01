...
{
	ifstream in("inputrequired.txt"); // имя файла и спецификатор (для чтения)
	in >> N;//столбцы
	in >> M;//стоки
...
void decreasSort(int numberOfPrime[100], int numReading[50][50], int &N, int &M) // сортировка
{
	...
	{
		int s = sI;
		while (s > 0 && numberOfPrime[s - 1] < numberOfPrime[s]) // бежим по элементам стоящим cлева, пока выполняется условие
		{
